...
@app.get(
    "/user/{user_id}/timesheet", response_model=int, status_code=status.HTTP_200_OK
)
...
@app.get(
    "/user/{user_id}/timesheet/summary",
    response_model=schemas.Timesheet,
...
def get_user_timesheet_summary(user_id: int):
    ...
    if timesheet is None:
        raise HTTPException(
            ...
            detail="User does not have a timesheet.",
        )
    return timesheet
...
@app.patch(
    "/user/{user_id}", response_model=schemas.User, status_code=status.HTTP_202_ACCEPTED
)
...
def check_online(user_id):
    ...
    entry = (
        ...
        .filter(models.Entry.timesheetID == timesheet.id)
        .filter(models.Entry.date == date)
        .first()
    )
...
@app.get(
    "/team/user/{user_id}", response_model=schemas.Team, status_code=status.HTTP_200_OK
)
...
@app.patch(
    "/team/{team_id}", response_model=schemas.Team, status_code=status.HTTP_202_ACCEPTED
)
...
@app.get(
    "/timesheet", response_model=List[schemas.Timesheet], status_code=status.HTTP_200_OK
)
def get_timesheets():
...
@app.post(
    "/timesheet", response_model=schemas.Timesheet, status_code=status.HTTP_202_ACCEPTED
)
def create_timesheet(timesheet: schemas.Timesheet):
    check_timesheet = (
        db.query(models.Timesheet)
        .filter(models.Timesheet.owner == timesheet.owner)
        .first()
    )
...
@app.get(
    "/entry/{entry_id}", response_model=schemas.Entry, status_code=status.HTTP_200_OK
)
...
# Clock in endpoint
@app.post(
    "/clock-in", response_model=schemas.Entry, status_code=status.HTTP_202_ACCEPTED
)
def clock_in(entry: schemas.Clock_req):
    ...
    timesheet = (
        db.query(models.Timesheet)
        .filter(models.Timesheet.id == entry.timesheetID)
        .first()
    )
    ...
    new_entry = models.Entry(
        date=date, time_in=time_in, time_out=None, timesheetID=entry.timesheetID
    )
...
# Clock out endpoint
@app.patch(
    "/clock-out", response_model=schemas.Entry, status_code=status.HTTP_202_ACCEPTED
)
def clock_out(new_entry: schemas.Clock_req):
    if new_entry.millis_out:
        # Convert to a readable datetime
        time_out = datetime.fromtimestamp(new_entry.millis_out / 1000.0)
    else:
        raise HTTPException(
    ...
    entry = (
        db.query(models.Entry)
        .filter(models.Entry.timesheetID == new_entry.timesheetID)
        .filter(models.Entry.date == time_out.strftime("%x"))
        .first()
    )
